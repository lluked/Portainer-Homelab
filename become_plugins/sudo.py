...
# Vendored override of ansible.builtin.sudo: upstream ansible-core (as of
# 2.21.3) fails become on hosts running sudo-rs, because sudo-rs wraps the
# custom -p prompt in its own "[sudo: ...] Password:" text instead of
# replacing the prompt outright, and check_password_prompt() only matches
# an exact prefix. This is the fix from ansible/ansible#86175, which has
# merged to devel but has no released ansible-core version yet (its stable
# backports were reverted). Auto-discovered from become_plugins/ next to
# main.yml, so it shadows the built-in "sudo" plugin for this project only.
