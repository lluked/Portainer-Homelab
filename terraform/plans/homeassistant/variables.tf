...
variable "lab_domain" {
  description = "Base domain this stack's Traefik router rule is served under (homeassistant.<lab_domain>). Optional - if unset (the default), read from secret/lab in Vault instead (see main.tf); Vault isn't contacted at all if this is set explicitly."
  type        = string
  default     = null
}
