...
resource "portainer_stack" "traefik_root_discovery" {
  ...
  stack_file_content = templatefile(
    ...
    {
      dynamic_dir       = "${var.install_dir}/${var.volume_mounts["dynamic_dir"]}"
      root_host_address = var.ssh_host
    }
  )
